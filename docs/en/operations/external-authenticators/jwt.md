...
You can reference the user either by their current username or by UUID. Using the username is more ergonomic:

```sql
-- Assign a settings profile using the current JWT username
ALTER SETTINGS PROFILE my_profile ADD TO 'JWT::ClickHouse::my-service-id::jane.doe::<claims-hash>';

-- Alternatively, assign by UUID
...
- **No access rights granted:** The permission limit may not be configured, or the referenced role or user may lack the required grants. Without a permission limit, JWT users receive no access.
