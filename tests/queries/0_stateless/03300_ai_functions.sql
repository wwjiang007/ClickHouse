...
SELECT aiGenerateEmbedding('ai_test'); -- { serverError NUMBER_OF_ARGUMENTS_DOESNT_MATCH }

SELECT '-- aiGenerateEmbedding: too many arguments';
SELECT aiGenerateEmbedding('ai_test', 'x', 256, 'extra'); -- { serverError NUMBER_OF_ARGUMENTS_DOESNT_MATCH }

SELECT '-- aiGenerateEmbedding: non-constant dimensions';
SELECT aiGenerateEmbedding('ai_test', x, toUInt64(number)) FROM (SELECT x, 0 AS number FROM _03300_input); -- { serverError ILLEGAL_COLUMN }

SELECT '-- aiGenerateEmbedding: wrong type for dimensions (signed integer)';
SELECT aiGenerateEmbedding('ai_test', x, -1) FROM _03300_input; -- { serverError ILLEGAL_TYPE_OF_ARGUMENT }

SELECT '-- aiGenerateEmbedding: wrong type for dimensions (string)';
SELECT aiGenerateEmbedding('ai_test', x, '256') FROM _03300_input; -- { serverError ILLEGAL_TYPE_OF_ARGUMENT }

SELECT '-- aiGenerateEmbedding: non-constant collection';
SELECT aiGenerateEmbedding(x, x) FROM _03300_input; -- { serverError ILLEGAL_COLUMN }
...
CREATE TABLE _03300_ret_embed ENGINE = Memory AS
    SELECT aiGenerateEmbedding('ai_test', x) AS result FROM _03300_input;
SELECT name, type FROM system.columns
...
CREATE TABLE _03300_ret_embed_dim ENGINE = Memory AS
    SELECT aiGenerateEmbedding('ai_test', x, 256) AS result FROM _03300_input;
SELECT name, type FROM system.columns
    WHERE database = currentDatabase() AND table = '_03300_ret_embed_dim';
DROP TABLE IF EXISTS _03300_ret_embed_dim;

SELECT '-- aiGenerateEmbedding: empty input executes';
SELECT count() FROM (SELECT aiGenerateEmbedding('ai_test', x) AS result FROM _03300_input);

SELECT '-- aiGenerateEmbedding: empty input with dimensions';
SELECT count() FROM (SELECT aiGenerateEmbedding('ai_test', x, 128) AS result FROM _03300_input);
...
-- =============================================================================
-- 16. Re-disable the setting mid-session
-- =============================================================================
